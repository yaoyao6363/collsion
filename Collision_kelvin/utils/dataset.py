...
from utils.data_viz_utils import DataVizUtils


class CreateDataset:

    def __init__(self, args, latest_cdm=True, n_latest_cdms=2):
        """
        ...
        requested_latest = getattr(args, 'n_latest_cdms', n_latest_cdms)
        if requested_latest < 2:
            raise ValueError("n_latest_cdms 需至少为 2，才能构建有效的序列。")
        self.n_latest_cdms = requested_latest

    def split_input_output_seq(self, data_frame):
        """
    ...
    def custom_train_test_split(self, data_frame, train_size=0.7, validation_size=0.5, test_size=0.3):
        """
        ...
        val_prop_within_20 = validation_size
    ...
    def get_train_test_dataset(self, data_frame):
        ...
        train_x_c = np.array(
            [DataUtils.pad_matrix(train[:, calssification_features], self.n_latest_cdms - 1).ravel() for train in
             train_x])
        train_y_c = np.array([train[:, calssification_output].ravel() for train in train_y]).reshape(-1).astype(int)
        test_x_c = np.array(
            [DataUtils.pad_matrix(test[:, calssification_features], self.n_latest_cdms - 1).ravel() for test in test_x])
