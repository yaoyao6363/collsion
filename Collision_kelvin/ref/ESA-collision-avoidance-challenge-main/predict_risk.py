...
from sklearn.metrics import fbeta_score, mean_squared_error, mean_absolute_error, r2_score
...
class PredictRisk:
    ...
    def _save_metrics_to_file(self, metrics: dict, classification_report: str):
        """
        Save evaluation metrics to a text file.
        
        Args:
            metrics (dict): Dictionary containing all evaluation metrics
            classification_report (str): Classification report string
        """
        output_file = os.path.join(os.getcwd(), 'evaluation_results.txt')
        
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write('='*70 + '\n')
            f.write(' '*20 + 'MODEL EVALUATION RESULTS\n')
            f.write('='*70 + '\n\n')
            ...
            f.write(f'  MAPE (Mean Absolute % Error):        {metrics["MAPE"]:.4f}%\n')
            f.write(f'  R²   (Coefficient of Determination): {metrics["R2"]:.6f}\n')
            
            f.write('\n[Classification Metric]\n')
            f.write(f'  F2-score (beta=2, threshold={THRESHOLD}):  {metrics["F2"]:.6f}\n')
            
            f.write('\n[Combined Metric]\n')
            f.write(f'  MSE/F2 Ratio (Original):             {metrics["MSE_F2_Ratio"]:.6f}\n')
        ...
        # Also save metrics as CSV for easy analysis
        csv_file = os.path.join(os.getcwd(), 'evaluation_metrics.csv')
    ...
    def evaluate_final_risk(self, gt_risk, pred_risk) -> dict:
        """
        Evaluates the final risk prediction performance with comprehensive metrics.
        ...
        Returns:
        dict: Dictionary containing MSE, RMSE, MAE, MAPE, R2, F2, and MSE/F2 ratio.

        Notes:
        - Computes regression metrics: MSE, RMSE, MAE, MAPE, R2
        - Computes classification metric: F2-score (based on threshold)
        - Returns comprehensive evaluation metrics
        """
        # Regression metrics (on all samples)
        ...
        # F2-score (classification metric based on threshold)
        gt_mask = gt_risk >= THRESHOLD
        pred_mask = pred_risk >= THRESHOLD
        f2 = fbeta_score(gt_mask, pred_mask, beta=2)
        
        # MSE on high-risk samples only (original metric)
        mse_high_risk = mean_squared_error(gt_risk[gt_mask], pred_risk[gt_mask]) if gt_mask.sum() > 0 else 0.0
        mse_f2_ratio = mse_high_risk / f2 if f2 > 0 else float('inf')
        ...
        return {
            ...
            'R2': r2,
            'F2': f2,
            'MSE_F2_Ratio': mse_f2_ratio  # Original evaluation metric
        }
    ...
    def run_pipe_line(self):
        ...
        print('\n[Classification Metric]')
        print(f'  F2-score (beta=2, threshold={THRESHOLD}):  {metrics["F2"]:.6f}')
        
        print('\n[Combined Metric]')
        print(f'  MSE/F2 Ratio (Original):             {metrics["MSE_F2_Ratio"]:.6f}')
        ...
        self._save_metrics_to_file(metrics, report)
